...
void lsp_locator::replace_text(lsp_range range, string8_view replacement_text,
  ...
  int end_line = std::min(narrow_cast<int>(this->offset_of_lines_.size()) - 1,
                          range.end.line);
  ...
  insert_back_transform(this->old_offset_of_lines_.begin() + end_line + 1,
  ...
  this->line_is_ascii_.insert(this->line_is_ascii_.end(),
                              this->old_line_is_ascii_.begin() + end_line + 1,
