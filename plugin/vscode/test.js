...
if (require.main === module) {
  console.error(
    `fatal: do not execute ${__filename} directly; run run-test-in-vscode.js instead`
  );
  process.exit(1);
}
...
let { vscodeTestMainAsync } = require("./test-support.js");

let vscode = require("vscode");
let qljsExtension = require("./extension.js");

let tests = {
  "load plugin after file opened": async ({ addCleanup }) => {
  ...
  "open existing file": async ({ addCleanup }) => {
    let scratchDirectory = makeScratchDirectory({ addCleanup });
    let helloFilePath = path.join(scratchDirectory, "hello.js");
    fs.writeFileSync(helloFilePath, "let x = 3;\nlet x = 4;\n");
    let helloURI = vscode.Uri.file(helloFilePath);
  ...
  // @@@ non-JS documents
  // @@@ different file extensions: .mjs, .cjs
  // @@@ plugin deactivate removes diagnostics
...
async function runAsync() {
  ...
  let extension = vscode.extensions.getExtension(
    "quick-lint.vscode-quick-lint-js"
  );
  await extension.activate();
  await qljsExtension.deactivate();

  await vscodeTestMainAsync(tests);
}
